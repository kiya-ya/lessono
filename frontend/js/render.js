...
function renderHallComparePage() {
  const perPage = 10;
  const total = hallCompareData.length;
  const totalPages = Math.ceil(total / perPage);
  const start = hallComparePage * perPage;
  const end = Math.min(start + perPage, total);
  const pageData = hallCompareData.slice(start, end);
  ...
  let html = `<span style="font-size:13px;color:#666;margin-right:12px;">共 ${total} 个大厅 · 第 ${hallComparePage + 1}/${totalPages} 页</span>`;
